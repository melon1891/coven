...
with st.sidebar:
    ...
    with st.expander("🌙 世界観", expanded=False):
        st.markdown("""
        **魔女協会**——それは、人知れず世界の均衡を守る者たちの組織。

        辺境の村々を魔物の脅威から守り、交易路を切り開き、
        次代の魔女を育成する。それが協会に課せられた使命。

        あなたは協会に所属する魔女の一人。
        限られた資源と人手の中で、いかに効率よく任務を遂行し、
        協会への貢献度（VP）を高められるかが問われている。

        しかし、協会内での立場は相対的なもの。
        他の魔女たちとの競争に勝ち抜き、
        最も優れた成果を示した者だけが、次の「大魔女」の座に就くことができる。

        *——魔法は万能ではない。だからこそ、知恵と戦略が必要なのだ。*
        """)
